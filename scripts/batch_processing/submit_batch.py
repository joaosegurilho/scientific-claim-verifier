...
def submit_batch(batch_file_path: str) -> str:
    ...
    for attempt in range(max_retries):
        try:
            uploaded_file = client.files.upload(
                file=str(input_file),
                config={"display_name": f"batch_{timestamp}_{part_num}", "mime_type": "application/jsonl"},
            )
    ...
    for attempt in range(max_retries):
        try:
            batch_job = client.batches.create(
                ...
                config={"display_name": f"proposition_extraction_{timestamp}_{part_num}"},
            )
    ...
    print("Estimated completion: 12-24 hours")
    print("\nTo monitor this job:")
    print(f"  python monitor_batch_jobs.py --job-id {batch_job.name}")
    print("\nOr monitor all jobs:")
    print("  python monitor_batch_jobs.py --list-all")
