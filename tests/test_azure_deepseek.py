...
logger = logging.getLogger("langchain-azure-deepseek-test")

logger.info("Starting LangChain Azure OpenAI (DeepSeek) hello-world test")
...
try:
    ...
    llm = Config.with_llm(model="deepseek-r1", temperature=0.0)
...
try:
    ...
    logger.info("Received response from Azure DeepSeek via LangChain")
except Exception:
    logger.exception("LangChain Azure DeepSeek invocation failed")
    sys.exit(1)
...
try:
    ...
    logger.info("Azure DeepSeek response text:")
    logger.info(content)
except Exception:
